...
compute_cw_dong <- function(DF, outcome_name="Y", treatment_name="A"){
  covariates_names <- !(names(DF) %in% c(treatment_name, "V", "S", outcome_name))
  tau_cw <- CW(moms.p = data.matrix(DF[DF$V == 0, covariates_names ]), moms.t = data.matrix(DF[DF$V == 1 , covariates_names]), dat.trial = DF[DF$V == 1,], type = "HT")  
  return(tau_cw)
}
...
compute_all <- function(DF, outcome_name="Y", treatment_name="A",
  ...
  # CW (only computed if dataset is complete)
  if (sum(is.na(temp[, !(names(temp) %in% c("V", "S", outcome_name, treatment_name))]))==0) {
    tau_cw_hat <- compute_cw_dong(temp, outcome_name = outcome_name, treatment_name = treatment_name)
  } else{
    tau_cw_hat <- NA
  }
